...
export function AssignStudentSequenceForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await writeStudentSequencePlans(db, trainerId, studentStorageId.trim(), programsInOrder, cycles);
      toast({ title: t("sequenceAssignedToast") });
      resetForm();
      onSuccess?.();
    } catch (e: unknown) {
  ...
  const controlsDisabled = disabled || isAssigning || !studentStorageId.trim();
      ...
      <div className="space-y-2">
            ...
            return (
              ...
              >
                ...
                <Button
                  ...
                  disabled={idx === 0 || disabled || isAssigning || isSavingTemplate}
                  aria-label="Move up"
                >
                ...
                <Button
                  ...
                  className="h-8 w-8 shrink-0"
                  onClick={() => moveProgram(idx, 1)}
                  disabled={idx === orderedProgramIds.length - 1 || disabled || isAssigning || isSavingTemplate}
        ...
        <div className="flex gap-2 flex-wrap">
          {assignablePrograms.length === 0 ? (
            ...
            <>
              <Select value={addPick || undefined} onValueChange={setAddPick} disabled={disabled || isAssigning || isSavingTemplate}>
              ...
              <Button
                ...
                variant="secondary"
                onClick={addProgram}
                disabled={!addPick || disabled || isAssigning || isSavingTemplate}
                className="gap-1"
              >
